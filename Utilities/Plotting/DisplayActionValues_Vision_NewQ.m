...
for kRow=1:nOSV(1)
    for kCol=1:nOSV(2)
        ...
        if multOutFlag==1
            tempQ=net(inpAll);
            tempQ2(:,:,:,:)=permute(reshape(tempQ,[3 s.wrld.size(1) s.wrld.size(2) s.wrld.size(2)]),[2 3 4 1]); % $$$ HERE - this is a guess right now
        else
            tempQ=net(inp);
