function [newR,newC,resetFlag, bodyTouch, teleportLR] = MoveObj(s, bodyCol, oldR,oldC,speedR,speedC, randomSpread)
...
% w.teleportLR is -1 if the object has been teleported to the left, and +1 if
% it has been teleported to the right
...
if numel(randomSpread)==1
    randomSpread = [randomSpread randomSpread];
end 

if strcmp(s.wrld.resetType,'BottomToTop') | strcmp(s.wrld.resetType,'BottomTop_InfLR')
	resetCondition = oldR > 1 & oldR < size(world2D,1)-1;
...
if resetCondition==1
    %             oldR=oldR+round(rand(1,1))+1;
    newR=oldR+speedR + round(randomSpread(1)*(rand(1,1)-0.5));
    if newR >=size(world2D,1)-1
        newR = size(world2D,1)-1;
    end
    if newR <=2
    ...
    oldCTemp=oldC+round(randomSpread(2)*(rand(1,1)-0.5)) + speedC; % $$$ UNDERSTAND THIS RANDOM MOTION AND PARAMETRISE IT
    newC=oldCTemp;
    if strcmp(s.wrld.resetType,'BottomToTop')
        while (oldCTemp < 2) | (oldCTemp > size(world2D,2)-1)
            oldCTemp=oldC+speedC + round(randomSpread(2)*(rand(1,1)-0.5)) ;
            if speedC~=0
                speedC=speedC-sign(speedC);
            end
        end
    ...
    elseif strcmp(s.wrld.resetType,'BottomTop_InfLR')
        
        if newC >size(world2D,2)-1
            newC = -size(world2D,2)+newC+2;
            teleportLR = -1;
        end
        if newC <2
            newC =  size(world2D,2)-newC-2;
...
if s.bdy.size>0
    ...
    % %     end
    if newR==size(world2D,1)-1 & newC >= bodyCol & newC < bodyCol + s.bdy.size
        bodyTouch(newC-bodyCol + 1 )=1;
    end
