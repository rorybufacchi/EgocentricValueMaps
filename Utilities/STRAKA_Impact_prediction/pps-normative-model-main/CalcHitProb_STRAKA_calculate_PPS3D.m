function [ hp ] = CalcHitProb_STRAKA_calculate_PPS3D(s)
% -------------------------------------------------------------------------
% Calculate hit probabilities and uncertainties. Based on Straka et al 2022
...
figure
%positions for which predictions are calculated $$$ HERE

[R C Z] = meshgrid([1 : s.wrld.size(1)] , ...
                   [1 : s.wrld.size(2)] , ...
                   [1 : s.wrld.size(3)] );
R = R.* s.clc.binW - s.clc.binW./2;
C = C.* s.clc.binW - s.clc.binW./2;
Z = Z.* s.clc.binW - s.clc.binW./2;
