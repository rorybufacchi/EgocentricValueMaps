...
handLocProbs_Learn_G         = GaussBlur(handLocProbs_Learn,10,5);
handLocProbs_Learn_FarRew_G  = GaussBlur(handLocProbs_Learn_FarRew,10,5);
handLocProbs_Learn_NearRew_G = GaussBlur(handLocProbs_Learn_NearRew,10,5);

%% $$$ Plot 'evolution of planning'

figure,
for ii = 1:1:20
    subplot(5,4,ii)
    imagesc(onTableQPosX, onTableQPosY, handLocProbs_NearRew(:,:,ii)); axis xy
    xlim([-50 50]);
    ylim([0 100]);
end

%% Find and display probabilities of ending up in each block
...
blockYsize = 49.81/6;
stimPosX   =             blockXsize .* -4   + blockXsize .* (1:7);
% stimPosY   = tableYMin + blockYsize .* -0.5 + blockYsize .* (1:6); 
% stimPosY   = blockYsize .* -0.25 + blockYsize .* (1:6); 
stimPosY   = 2.5 + blockYsize .* (1:6); 
...
% % %         endProbs(iSY,iSX) =  mean(...
% % %                              handLocProbs(onTableQPosY >= yBinMin & onTableQPosY <= yBinMax, ...
% % %                                           onTableQPosX >= xBinMin & onTableQPosX <= xBinMax,cTP), [1 2]);

        sumProbs = 0;
        for iSmallerBin = 1:numel(rr)
             sumProbs = sumProbs + sum(handLocProbs_NearRew(rr(iSmallerBin), cc(iSmallerBin),cTP));
        end
        ...
        % experimental block
        endProbs(iSY,iSX) = ((blockXsize .* blockYsize) ./ 25) .* (sumProbs ./ numel(rr));
    end
end
...
% figure,
subplot(2,1,1)
imagesc(stimPosX,stimPosY,endProbs)
axis xy
colorbar

subplot(2,1,2)
imagesc(onTableQPosX, onTableQPosY, handLocProbs_NearRew(:,:,cTP));
axis xy
colorbar
colormap jet
...
% Control
pC = squeeze(mean(handLocProbs_Learn_G(:,:,cDiffuseTP,cLearnTP,cAlpha,cTemp), 4));
% Far
pF = squeeze(mean(handLocProbs_Learn_FarRew_G(:,:,cDiffuseTP,cLearnTP,cAlpha,cTemp), 4));
% Near
pN = squeeze(mean(handLocProbs_Learn_NearRew_G(:,:,cDiffuseTP,cLearnTP,cAlpha,cTemp), 4));
...
lateBlocks  = 31:33;
% Control
pEarlyC = squeeze(handLocProbs_Learn_G(:,:,cDiffuseTP,earlyBlocks,cAlpha,cTemp));
pLateC  = squeeze(handLocProbs_Learn_G(:,:,cDiffuseTP,lateBlocks, cAlpha,cTemp));
% Far
pEarlyF = squeeze(handLocProbs_Learn_FarRew_G(:,:,cDiffuseTP,earlyBlocks,cAlpha,cTemp));
pLateF  = squeeze(handLocProbs_Learn_FarRew_G(:,:,cDiffuseTP,lateBlocks, cAlpha,cTemp));
% Near
pEarlyN = squeeze(handLocProbs_Learn_NearRew_G(:,:,cDiffuseTP,earlyBlocks,cAlpha,cTemp));
pLateN  = squeeze(handLocProbs_Learn_NearRew_G(:,:,cDiffuseTP,lateBlocks, cAlpha,cTemp));




real1DReachDist = {[0.619 0.535 0.481] ; ... Reaching distance during 1st blocks
                   [0.698 0.528 0.420]};... Reaching distance during last blocks

% Highest estimate
real1DReachDistStd = {[0.740 0.632 0.586] ; ... Reaching distance during 1st blocks
                      [0.783 0.620 0.520]};... Reaching distance during last blocks
% Actual proper units
for iD = 1 : numel(real1DReachDist)
    real1DReachDist{iD}    = real1DReachDist{iD}    .* 45.50 ;
    real1DReachDistStd{iD} = real1DReachDistStd{iD} .* 45.50 ; 
end
% Correct the values to the right units
for iD = 1 : numel(real1DReachDist)
    real1DReachDistStd{iD} = real1DReachDistStd{iD} - real1DReachDist{iD};
end
...
figure,
subplot(1,2,1);
errorbar([0.85 1.15 ; 1.85 2.15 ; 2.85 3.15] , cell2mat(real1DReachDist(1:2))', cell2mat(real1DReachDistStd(1:2))','.k');
hold on;
bar(cell2mat(real1DReachDist(1:2))')
ylim([0 40])

subplot(1,2,2);
modelM  = [meanEarlyReachF meanEarlyReachC meanEarlyReachN ; ...
           meanLateReachF  meanLateReachC  meanLateReachN   ]';
modelSD = [stdEarlyReachF  stdEarlyReachC  stdEarlyReachN ; ...
           stdLateReachF   stdLateReachC   stdLateReachN   ]';
errorbar([0.85 1.15 ; 1.85 2.15 ; 2.85 3.15], modelM, modelSD,'.k');
hold on;
bar(modelM);
ylim([0 40])
...
% Control
pProxC = squeeze(sum(handLocProbs_Learn_G(proxPositions,:,cDiffuseTP,:,cAlpha,cTemp),[1 2]));
pDistC = squeeze(sum(handLocProbs_Learn_G(distPositions,:,cDiffuseTP,:,cAlpha,cTemp),[1 2]));
% Far Rewards
pProxF = squeeze(sum(handLocProbs_Learn_FarRew_G(proxPositions,:,cDiffuseTP,:,cAlpha,cTemp),[1 2]));
pDistF = squeeze(sum(handLocProbs_Learn_FarRew_G(distPositions,:,cDiffuseTP,:,cAlpha,cTemp),[1 2]));
% Near Rewards
pProxN = squeeze(sum(handLocProbs_Learn_NearRew_G(proxPositions,:,cDiffuseTP,:,cAlpha,cTemp),[1 2]));
pDistN = squeeze(sum(handLocProbs_Learn_NearRew_G(distPositions,:,cDiffuseTP,:,cAlpha,cTemp),[1 2]));
...
pCblur = GaussBlur(repmat(pC,[1 1 2 2]), 1, 1);
