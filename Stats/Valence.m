...
%%
% % % tmpA = structMetricByNetType(1,:) - permute(structMetricByNetTypePm(1,:,:),[1 3 2]);
% % % tmpB = structMetricByNetType(2,:) - permute(structMetricByNetTypePm(2,:,:),[1 3 2]);

tmpA = structMetricByNetType(1,:) - nanmean(permute(structMetricByNetTypePm(1,:,:),[1 3 2]),3);
tmpB = structMetricByNetType(2,:) - nanmean(permute(structMetricByNetTypePm(2,:,:),[1 3 2]),3);

[h  p ci stats] = ttest2(tmpA(:), tmpB(:))

[p h stats] = ranksum(tmpA(:), tmpB(:))

% $$$ Try a correlation
[rho pval] = corr(allNetW',structMetric - nanmean(structMetricPm,1)' )

% Try an LME
tbl                             = table(structMetric - nanmean(structMetricPm,1)' );
tbl.Properties.VariableNames    = {'StructMetric'};
tbl.netOutputW                  = allNetW'; 


lme = fitlme(tbl,'StructMetric ~ netOutputW + 1')
...
for iM = 1:length(allNetAR)
    ...
    [rho(iM) pval(iM)] = corr(stimPrefSimTmp(:),1./nodeDistsTmp(:));
...
sprintf('%i out of %i networks show structure using this metric', sum(pValAdj(4:48) < 0.05), 45);
...
% tbl.layNumDiff                  = categorical(layNumDiff(:));
