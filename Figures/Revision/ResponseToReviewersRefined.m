...
for iAct = 1:6
figN = ['TwoDimPlotQAct' num2str(iAct)];
...
for iF = 1:length(allFields)5
    ...
    set(f.(cF).f, 'Renderer', 'painters'); % default, opengl
    saveas(f.(cF).f,['C:\Users\Rory Bufacchi\OneDrive\Projects\DPPS\DefenseAgent\Documentation\Figures\Revision\Wasp\' cF '.eps'] , 'epsc')
    saveas(f.(cF).f,['C:\Users\Rory Bufacchi\OneDrive\Projects\DPPS\DefenseAgent\Documentation\Figures\Revision\Wasp' cF '.pdf'] , 'pdf')
...
%% Finalise pca stants and plot scatter plus histograms
...
for iMet = 1:length(metDefs)
    ...
    ax{1} = axes('Position',[axS.xOffset + ...
        ...
        axS.ySpace + (iRow-1) .* ((1 - 2.*axS.yOffset) ./ length(rowDefs)), ...
    ...
    ax{2} = axes('Position',ax{1}.Position .* [1 1 1 0] + [0 -axS.ySpace.*.9 0 axS.ySpace.*.9]  );
    histogram(tmpX,10);
    set(ax{2},'ydir','r')
    box off
    xlim(xLims);
    ...
    histogram(tmpY,10,'orientation','horizontal');
    set(ax{3},'xdir','r')
    box off
    ylim(yLims);
end
