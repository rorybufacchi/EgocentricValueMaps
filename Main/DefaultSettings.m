...
% DefaultSettings Sets all settings that haven't been specified to default
% values
...
s.fl.mo                     = 1;% Flag for multiple outputs. If 1, it gives 1 output for each action. If 0, actions are inputs
s.fl.vid                    = 0; % Record videos - CAREFUL - might have unexpected behaviour
s.fl.dspm                   = 0; % 1 means display world and 0 means no display
s.fl.rdat                   = 1; % This is if the relearning should only use more recent data
s.fl.os                     = 1; % Oversample high and low reward cases
...
s.lp.minExp                 = 2e6; % Minimum number of epochs before first training starts
...
s.plt.titleFl               = 1; % set to 0 if you don't want to plot Titles
...
% -------------------------------------------------------------------------
% Settings for graph/network analysis and plotting
...
s.clc.startSZ               = 1; % Limb height (not important in 2D)
s.clc.nReps                 = 1; 
...
s.clc.thinLimbsFl           = 0; % Whether to assume the limb is only as thick as the area that is defined to give a reward, or extends further back
s.clc.rewardInterceptFl     = 1; % Whether to reward 'moving through' a rewarded area, rather than just 'landing on' a rewarded area
s.clc.nSteps                = 1; 
s.clc.gammaVal              = 0.8;
s.clc.baseVel               = [4  0  0]; 
...
if numel(varargin)>0
    ...
    for iFN = 1:length(sFields)  %field name counter
        % If the substrucure doesn't exist, put it there in its entirety
        if ~isfield(sNew,sFields{iFN})
            sNew.(sFields{iFN}) = s.(sFields{iFN});
        else
            % If the substructure does exist, loop through the
            % substructure to check whether there are any missing bits
